...
DECLARE @Message_FROM_WAT_User		NVARCHAR(128)	= 'CUST_MA@upshealthcare.com'				-- A felado user azonositoja. Ha e-mail cim, akkor az, ha telefonszam, akkor az.
DECLARE @Message_TO_WAT_User		NVARCHAR(128)	= '0036303102211'							-- A cimzett user azonositoja. Ha e-mail cim, akkor az, ha telefonszam, akkor az.
DECLARE @Message_Type				INT				= 50001										-- 50001 = TOUREXPORT
																								-- 50002 = Transport Order Statuschange Notification
																								-- 50003 = Accepting downloaded messages
DECLARE @WAT_Message				NVARCHAR(MAX)	= N'Hello Istvan, ide jon majd a JSON-od!'
...
SELECT 'WAT_Messages All', * FROM WAT_Messages	-- Itt lehet megfigyelni, hogy az uj message lezarja a korabbiakat. Tehat mindig csak 1 aktualis 50001-es tipusu message van nyitva.
