...
class Auth {
  ...
  async registerUser() {
    let crypto = new Crypto.Crypto();
    let passwordHash = await crypto.getHashedPassword(
      this.comm.req.req.body.body.password
    );
      ...
      let token = jwt.sign(
        {
          ...
          _hashShorthand: this.comm.req.req.body.body.passwordHash.substring(
            0,
            6
          ),
  ...
  async getUserParams(portalOwnersId, userId) {
    return await this.sp.WAT_INTERFACE_getUserParams({
      portalOwnersId,
      userId,
    });
  }
  ...
  async extendTokenValidity() {
    ...
    let decodedToken = await this.decodeToken(
      portalOwnerId,
  ...
  async changePassword() {
    ...
    let outParams = await this.sp.WAT_INTERFACE_changePassword({
      ...
      newPassword_updateRequired:
        this.comm.req.req.body.body.newPassword_updateRequired,
    });
    if (outParams.result === true) {
      this.comm.res.setResultOk({});
      return;
