...
class StoredProcedures {
  ...
  async WAT_INTERFACE_extendTokenValidity(params) {
    ...
    try {
      let dbResults;
      dbResults = await this.db.spExecute(
        dbRequest,
        "WAT_INTERFACE_extendTokenValdity"
      );
  ...
  async WAT_INTERFACE_changePassword(params) {
    ...
    dbRequest.input("userId", npm_mssql.Int, params.userId);
    dbRequest.input(
      "currentToken_hashShorthand",
      npm_mssql.NVarChar(1024),
      params.currentToken_hashShorthand
    );
    dbRequest.input(
      "currentToken_salt",
      npm_mssql.VarChar(50),
      params.currentToken_salt
    );
    dbRequest.input(
      "newPassword_hash",
      npm_mssql.VarChar(1024),
      params.newPassword_hash
    );
    dbRequest.input(
      "newPassword_updateRequired",
      npm_mssql.Bit,
      params.newPassword_updateRequired
    );
  ...
  async WAT_INTERFACE_getRecordsetParams(params) {
    ...
    try {
      let dbResults = await this.db.spExecute(
        dbRequest,
        "WAT_INTERFACE_GET_RECORDSET_PARAMS"
      );
